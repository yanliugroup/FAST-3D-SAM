...
def main():
    ...
    parser.add_argument(
        "--method", default=None, type=str, choices=["sam", "baidu", "tri_attn_loraPtuning_pEncodeS_miniDe"]
    )
    parser.add_argument(
    ...
    elif args.method == "tri_attn_loraPtuning_pEncodeS_miniDe":
        from modeling.Med_SAM.image_encoder_tri_attn_Lora_ptuning import ImageEncoderViT_3d_v2 as ImageEncoderViT_3d
    ...
    if args.method == "baidu":
        ...
        for p in img_encoder.slice_embed.parameters():
            p.requires_grad = True
        for i in img_encoder.blocks:
            i.prompt_embeddings.requires_grad = True
            for p in i.norm1.parameters():
                p.requires_grad = True
            for p in i.norm2.parameters():
                p.requires_grad = True
    ...
    for epoch_num in range(args.max_epoch):
        ...
        for idx, (img, seg, spacing) in enumerate(train_data):
            ...
            if use_click:
                ...
                click_num = random.randint(1, 12)
                #print("using click", click_num)

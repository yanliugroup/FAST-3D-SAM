...
class ImageEncoderViT_3d_v2(nn.Module):
    ...
    def forward(self, x: torch.Tensor, batchsize = 1, points_feat = None) -> torch.Tensor:
        # x: 256, 3, 256, 256
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.down_layer = nn.Upsample(scale_factor=0.5, mode='trilinear')
        self.up_layer = nn.Upsample(scale_factor=2, mode='trilinear')
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # H, W, hidden_dim = x.shape[1], x.shape[2], x.shape[3]
        # D = H
        # x_g = x.reshape(-1, D, H, W, hidden_dim)
        # x_g = self.down_layer(x_g.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1)
        # x_g = x_g.reshape(-1, D // 2, H*W // 4, hidden_dim)
        # x_g = self.attn(x_g)
        # x_g = x_g.reshape(-1, D // 2, H // 2, W // 2, hidden_dim)
        # x_g = self.up_layer(x_g.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1)
        # x_g = x_g.reshape(-1, H, W, hidden_dim)
        ...
        # hh, ww, hidden_dim = x.shape[1], x.shape[2], x.shape[3]
        # depth = 16
        ...
        #print(x.shape, shortcut.shape, x_g.shape)
        x = shortcut + x #+ x_g
        xn = self.norm2(x)

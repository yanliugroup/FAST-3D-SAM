...
class Inception3D(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv3d(
            ...
            kernel_size=(5,5,1), padding=(2,2,0), groups=mid_dim//group_scale)
        self.conv2 = nn.Conv3d(
            in_channels = mid_dim, 
            out_channels = mid_dim, 
            kernel_size=(1,1,5), padding=(0,0,2), groups=mid_dim//group_scale)
        
        self.conv3 = nn.Conv3d(
            in_channels = mid_dim, 
            out_channels = mid_dim, 
            kernel_size=3, padding=1, groups=mid_dim//group_scale)
        
        self.warmup = warmup
        if warmup:
            self.channel_scaler1 = torch.nn.Parameter(torch.ones(1, mid_dim, 1, 1, 1)*1e-2, requires_grad=True)
            self.channel_scaler2 = torch.nn.Parameter(torch.ones(1, mid_dim, 1, 1, 1)*1e-2, requires_grad=True)

    def forward(self, x):
        x1 = F.gelu(self.conv1(x))
        x1 = F.gelu(self.conv2(x1))
        
        x2 = F.gelu(self.conv3(x))
        
        if self.warmup:
            return x + self.channel_scaler1 * x1 + self.channel_scaler2 * x2
        else:
            return x + x1 + x2

class Adapter_Out(nn.Module):
    def __init__(
            self,
            input_dim,
            mid_dim,
        ...
        self.conv = Inception3D(mid_dim, True)
    ...
    def forward(self, features):
        ...
        out = features.permute(0, 4, 1, 2, 3)
        out = self.conv(out)
...
class Adapter_MultiHead(nn.Module):
    def __init__(
            self,
            input_dim,
            scale=2,
        ...
        super().__init__()
        head_dim = input_dim // head
        squeeze_dim = input_dim // scale
        self.linear1 = nn.Linear(head_dim, head_dim // scale)
        self.conv = Inception3D(squeeze_dim, warmup=warmup)
        self.linear2 = nn.Linear(head_dim // scale, head_dim)
        self.head=head
        self.warmup=warmup
        if warmup:
            self.channel_scaler = torch.nn.Parameter(torch.ones(1, 1, 1, 1, input_dim)*1e-2, requires_grad=True)

    def forward(self, features):
        
        bs, h, w, d, dim = features.shape
        ...
        out = self.linear2(out)
        out = out.reshape(bs,h,w,d,-1)
        out = F.gelu(out)
        
        if self.warmup:
            out = features + self.channel_scaler * out
        else:
            out = features + out
...
class ImageEncoderViT_3d_v2(nn.Module):
    def __init__(
        ...
        self.neck_3d = Adapter_Out(768, out_chans * 2, out_chans)

    def forward(self, x: torch.Tensor, batchsize) -> torch.Tensor:
        with torch.no_grad():
            x = self.patch_embed(x)
        bsxD, H, W, C = x.shape
        if self.num_slice > 1:
            x = self.slice_embed(x.reshape(batchsize, -1, H, W, C).permute(0, 4, 2, 3, 1))  # 1, C, H, W, D做3D Conv
        ...
        if self.pos_embed is not None:
            ...
            pos_embed = F.interpolate(pos_embed.float().permute(0, 4, 1, 2, 3), size=x.shape[2], mode='trilinear').permute(0, 2, 3, 4, 1)  # TODO: this change for 256 input
            x = x + pos_embed
...
class Block_3d(nn.Module):
    ...
    def __init__(
        ...
        if self.shift_size > 0:
            # H, W, D = 32, 32, 32
            H, W, D = 16, 16, 16  # TODO: change this
        ...
        self.adapter = Adapter_MultiHead(input_dim=dim)
...
class Attention_3d(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask=None) -> torch.Tensor:
        ...
        qkv = self.qkv(x.reshape(B, H*W*D, -1)).reshape(B, H * W * D, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)

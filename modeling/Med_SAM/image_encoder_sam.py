...
class ImageEncoderViT_3d_v2(nn.Module):
    def __init__(
        ...
        self.neck_3d = nn.Sequential(
                nn.Conv3d(embed_dim, out_chans, kernel_size=1, bias=False),
                nn.InstanceNorm3d(out_chans),
                nn.ReLU(),
                nn.Conv3d(out_chans, out_chans, kernel_size=3, bias=False, padding=1),
                nn.InstanceNorm3d(out_chans),
                nn.ReLU()
                )
    ...
    def forward(self, x: torch.Tensor, batchsize) -> torch.Tensor:
        ...
        # NOTE: here x shape is: bs, H, W, D, C
        if self.pos_embed is not None:
            ...
            x = x + pos_embed
        # print("x add pos", x.shape, "pos", pos_embed.shape)
        idx = 0
        ...
        x = self.neck_3d(x.permute(0, 4, 1, 2, 3))  
        return x
...
class Block_3d(nn.Module):
    ...
    def __init__(
        ...
        if self.shift_size > 0:
            # H, W, D = 32, 32, 32
            H, W, D = 16, 16, 16  # TODO: change this if image size is changed

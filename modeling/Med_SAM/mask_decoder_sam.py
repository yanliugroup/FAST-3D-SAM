...
class MaskDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.output_upscaling = nn.Sequential(
            ...
            activation(),
            #nn.Conv3d(transformer_dim // 8, 2, kernel_size=1, stride=1)
        )
    ...
    def predict_masks(
        ...
        if not sparse_prompt_embeddings is None:
            # print(output_tokens.shape, sparse_prompt_embeddings.shape, image_embeddings.shape)
            print(output_tokens.shape, sparse_prompt_embeddings.shape)
        ...
        # src = src# + dense_prompt_embeddings  # TODO: use mask
        ...
        # print("upscaled_embedding shape", upscaled_embedding.shape)
        # print("hyper_in shape", hyper_in.shape)
        masks = (hyper_in @ upscaled_embedding.view(b, c, h * w * d)).view(b, -1, h, w, d)
        #print("masks", masks.shape)
        masks = F.interpolate(masks, scale_factor = 2, mode='trilinear', align_corners=True)
        #print("masks", masks.shape)
        #raise "s"
        return masks

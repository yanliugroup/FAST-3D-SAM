...
def main():
    ...
    os.makedirs(f"Online_TrainPredict_0707/2/{args.data}", exist_ok=True)
    ...
    for idx, (img, seg, spacing) in enumerate(val_data):
        ...
        for _sim_idx in range(40):
            ...
            debug_slice = click_pos[0][0]
            
            with torch.no_grad():
                point_feature = prompt_encoder(prompt_input)
                batch_features = img_encoder(input_batch, batchsize, point_feature)  # bs, C, H, W, D
                masks, decoder_feats = mask_decoder(batch_features)
            masks = masks.permute(0, 1, 4, 2, 3)  # bs, C, D, H, W   [2, 2, 128, 128, 128]
            
            loss = dice_loss(masks, seg)
            print(_sim_idx, "loss", round(float(loss), 5))
        
        new_click_disk = new_positive_feat
        
        debug_slice = torch.argmax(seg[0][0].sum(-1).sum(-1))
        ...
        if args.save_result == 'yes':
            torch.save(
                ...
                , f"Online_TrainPredict_0707/2/{args.data}/data_{idx}.pt")
    ...
    logger.info("- metrics: " + str(np.mean(loss_summary)))
